...
class Recorder : IDisposable
{
    public Recorder(Settings settings)
    {
        ...
        if (!string.IsNullOrEmpty(_settings.StudySetupFilename))
        {
            ...
            var session = config.CreateSession(_participantId);
            if (session == null)
            {
    ...
    private void RunSession(Session session)
    {
        ...
        for (int i = 0; i < session.Blocks.Length; i++)
        {
            ...
            if (!_hasInterrupted)
            {
                var qas = session.QuestionsAndAnswers[block];
                foreach (var qa in qas)
                {
                    Console.WriteLine(string.Join('\n', qa.Question.GetQuestionTextLines()));
                    qa.Answer = qa.Question.ReadAnswer();
                    App.Debug.WriteLine("QUESTION", $"{qa.Question.ID}\t{qa.Answer}");
                }
            }
            else
            {
        ...
        if (!_hasInterrupted)
        {
            session.SaveConditionsAndAnswers(_settings);
